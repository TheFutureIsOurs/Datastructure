...
document.writeln("<div id=\"stat-z\" style=\"display: none;\">"); (function() {
	var td = new Date(),
	im = new Image,
	im_pv = new Image,
	mk = Math,
	...
	ac = 'clientac',
	wd = rf = rd = u = ds = "",
	ur = document.referrer.replace(/#/g, "~"),
	acl = encodeURI(document.location.href.replace(/#/g, "~")),
	r = /http:\/\/([^\/]+)\//i,
	...
	P('clickType', '0');
	//抓取文章标题title
	P('title', document.title) if (canSendZpng) {
		//处理cookie存储搜索词的问题
		if (wd != "") {
			S('cookie_wd', wd, 5) S('cookie_wd_redoman', rd, 5)
		}
		P('cookie_wd', C('cookie_wd') + 'cookie_wd') P('cookie_wd_redoman', C('cookie_wd_redoman') + 'cookie_wd_redoman') P('tempid', Math.random())
		//记录并处理新老访客cookie
		if (!C('visit_dt')) {
	...
	if (window.location.host != 'club.xywy.com') {
		im_pv.src = "http:\/\/stat-w.xywy.com\/w.png?Xt" + d['title'] + meta_descript + "Xt";
	}
	//}
})();
